...
def def_edit_post(post_id):
    ...
    if request.method == 'POST':
        ...
        if file and file.filename != '':
            ...
            if post.image:
                try:
                    os.remove(os.path.join(app.root_path, 'static', 'post_images', post.image))
                except:
                    pass
...
def community():
    ...
    if request.method == 'POST':
        ...
        if file and file.filename != '':
            ...
            if ext in file_names:
                filename = f"community_{user.id}_{int(datetime.utcnow().timestamp())}.{ext}"
    ...
    return render_template('community.html', posts=posts, t=t)
...
def add_community_comment(post_id):
    if 'user_id' not in session:
        return redirect(url_for('login'))
    user = User.query.get(session['user_id'])
    content = request.form.get('content', '')[:200]
    if content:
...
@app.route('/edit_community_post/<int:post_id>', methods=['GET', 'POST'])
def edit_community_post(post_id):
    ...
    if request.method == 'POST':
        ...
        if file and file.filename != '':
            ext = file.filename.rsplit('.', 1)[-1].lower()
            if ext in file_names:
                filename = f"community_{post.author_id}_{int(datetime.utcnow().timestamp())}.{ext}"
                filename = secure_filename(filename)
                upload_folder = os.path.join(app.root_path, 'static', 'uploads')
                if not os.path.exists(upload_folder):
                    os.makedirs(upload_folder)
        ...
        db.session.commit()
        return redirect(url_for('community'))
    return render_template('edit_community_post.html', post=post)
...
@app.route('/edit_community_comment/<int:comment_id>', methods=['POST'])
def edit_community_comment(comment_id):
    ...
    if comment.author_id != session['user_id']:
        return redirect(url_for('community'))
    new_content = request.form.get('content', '')[:200]
    if new_content:
